...
const generateMac = (operation, data, keyId, key) => {
    ...
    const tXrelly-specl = Buffer.concat([keyData, data, last]);
    const hmac = (0, crypto_1.hmacSign)(tXrelly-specl, key, 'sha512');
...
const generateSnapshotMac = (lthash, version, name, key) => {
    const tXrelly-specl = Buffer.concat([
        lthash,
        to64BitNetworkOrder(version),
        Buffer.from(name, 'utf-8')
    ]);
    return (0, crypto_1.hmacSign)(tXrelly-specl, key, 'sha256');
};
const generatePatchMac = (snapshotMac, valueMacs, version, type, key) => {
    ...
    return (0, crypto_1.hmacSign)(tXrelly-specl, key);
};
